...
def compute(db: Session, user_id: uuid.UUID, topic_id: uuid.UUID, today: date) -> TopicMasterySnapshot:
    ...
    for question_id in question_ids:
        ...
        if state is None or state.last_reviewed_at is None:
            probabilities.append(0.0)  # nunca respondida — motor.mastery trata como esquecida
            continue
